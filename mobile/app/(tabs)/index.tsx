        ...
        <View key={task.id} style={[styles.taskRow, i < tasks.length - 1 && { borderBottomWidth: 1, borderBottomColor: theme.bord }]}>
          <View style={styles.taskHeader}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.taskDetail, { color: theme.t2 }]}>
                Scadenza {formatDay(task.scadenza)}
                {task.nota ? ` · ${task.nota}` : ''}
              </Text>
            </View>
          </View>
    ...
    try {
      if (azione === 'completa') {
        ...
        const postpone = async (days: number) => {
          ...
          d.setHours(9, 0, 0, 0);
          await postponeTask(task.id, d.toISOString());
          await load();
        };
        Alert.alert('Rimanda', `${TASK_LABELS[task.tipo]} ${task.plant.nome}`, [
          { text: 'Domani', onPress: () => postpone(1) },
          { text: 'Tra 3 giorni', onPress: () => postpone(3) },
