...
### ✅ Risolto: bug collega/scollega pianta nel dettaglio vaso (D2, 2026-08-14)
Causa reale: `updatePlant` scollegava/collegava il vaso in due query separate senza transazione — se la prima falliva, o lo stato React era stale, restava agganciato alla pianta sbagliata o falliva con `VASE_ALREADY_LINKED` anche su cambio pianta legittimo. Non era Docker Desktop come sospettato. Fix: transazione Prisma unica in `plant.service.ts` (`f8539f8`).

### MEV-05 completa e testata (2026-08-18)
Riconfigurazione WiFi senza ri-pairing testata end-to-end su hardware reale: nessuna riga `smart_vases` duplicata, pianta collegata e storico letture intatti dopo la riconnessione.

### MEV-03 in pausa (2026-08-18)
Riordino vasi lato mobile fermo — due librerie drag provate e scartate (vedi `fiora-mev.md`). Backend (`ordine` + `PATCH /vases/order`) pronto e inutilizzato. Non prioritario per ora.

### MEV-08 — step 1 completo e testato (2026-08-18): soglie sensori per-pianta
Prima le soglie umidità/luce/temperatura vivevano solo su `Species` (condivise per specie, non per pianta reale). Ora `Plant` ha 6 campi nullable: `sogliaUmiditaMin/Max` (Int, %), `sogliaLuceMin/Max` (Int, lux), `sogliaTempMin/Max` (**Decimal(4,1)**, non Int — la temperatura reale ha un decimale, corretto in corsa dopo un test fallito). `PATCH /plants/:id` li accetta, richiede `vasoId` valorizzato (altrimenti `PLANT_NO_VASE`), valida min≤max; scollegare il vaso azzera le soglie. Mobile: `plant/[id].tsx` → "Personalizza soglie" (solo con vaso) → `SoglieVaseModal` (vuoto = default specie, temperatura accetta virgola o punto). `plantUi.ts` generalizzato a min+max con fallback pianta→specie→default hardcoded. Corretti in corsa anche due problemi UX pre-esistenti negli stessi file: `SensorTile` non mostrava il nome della metrica (aggiunto `title`), area di tocco del bottone soglie non tap-friendly.
**Lezione**: dopo `prisma migrate dev`/`generate`, riavviare sempre `npm run dev` — `ts-node-dev` non ricarica il Prisma Client nativo, causava 500 silenziosi (client vecchio in memoria senza i campi nuovi) nel primo giro di test.
**Step 2 (alert da sensore, = Fase 7)**: non ancora affrontato, vedi `docs/fiora-mev.md`.
...
- **Bug D2 risolto** (2026-08-14): vedi nota a inizio file — collega/scollega pianta da `vase/[id].tsx` ora in transazione unica
